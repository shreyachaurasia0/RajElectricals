  ...
  const fetchProduct = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setWarranty(product.warranty || "1 Year");
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      <div>
        ...
        <div className='flex gap-4'>
          <label htmlFor="image1">
            <img className='w-20' src={image1 ? URL.createObjectURL(image1) : (existingImages[0] || assets.upload_area)} alt="img1" />
            <input onChange={(e) => onImageChange(e, setImage1)} type="file" id="image1" hidden />
          </label>
          <label htmlFor="image2">
            <img className='w-20' src={image2 ? URL.createObjectURL(image2) : (existingImages[1] || assets.upload_area)} alt="img2" />
            <input onChange={(e) => onImageChange(e, setImage2)} type="file" id="image2" hidden />
          </label>
          <label htmlFor="image3">
            <img className='w-20' src={image3 ? URL.createObjectURL(image3) : (existingImages[2] || assets.upload_area)} alt="img3" />
            <input onChange={(e) => onImageChange(e, setImage3)} type="file" id="image3" hidden />
          </label>
          <label htmlFor="image4">
            <img className='w-20' src={image4 ? URL.createObjectURL(image4) : (existingImages[3] || assets.upload_area)} alt="img4" />
      ...
      <div className='w-full'>
        ...
        <textarea onChange={(e) => setDescription(e.target.value)} value={description} className='border border-gray-300 px-3 py-2 w-full rounded mt-2' placeholder='Write Product Description here...' required />
      </div>

      {/* ...existing specification, features, brand, warranty fields... */}
      <div className='w-full'>
        <p className='mb-2'>Product Specifications</p>
        <textarea onChange={(e) => setSpecifications(e.target.value)} value={specifications} className='border border-gray-300 px-3 py-2 w-full rounded mt-2' placeholder='Color: Black | Material: Glass | Power: 2000W' />
      </div>
      ...
      <div className='flex gap-4 w-full'>
        ...
        <div className='w-40'>
          ...
          <input onChange={(e) => setDeliveryCharge(e.target.value)} value={deliveryCharge} className='w-full px-3 py-2' type="number" min="0" step="0.01" placeholder='0.00' />
        </div>
      </div>

      {/* ...existing category, subcategory fields... */}
      ...
      <div className='flex gap-2 mt-2'>
        <input onChange={(e) => setBestseller(!bestseller)} checked={bestseller} type="checkbox" id='bestseller' />
      ...
      >
        {loading ? (
          <>
            ...
            {uploadProgress > 0 ? `${uploadProgress}%` : 'Updating...'}
          </>
        ) : (
          'UPDATE PRODUCT'
        )}
