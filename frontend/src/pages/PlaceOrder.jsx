...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try{
      ...
      let orderData = {
        address: formData,
        items: orderItems,
        amount : getCartAmount(), // subtotal; backend will compute per-item deliveryTotal
      }
      switch(method){
        case 'cod': {
        ...
        case 'razorpay': {
          const response = await axios.post(backendUrl + '/api/order/razorpay', orderData, {headers:{Authorization: `Bearer ${token}`}});
          if(response.data?.success){
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-30 sm:pt-30 min-h-[80vh] border-t'>
      {/*----------Left Side-----------*/}
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
          <div className='texl-2xl sm-text-2xl my-3'>
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12  '>
          ...
          {/*Payment Method */}
          <div className='flex gap-3 flex-col lg:flex-row pt-6'>
          ...
          <div className='w-full text-end mt-8'>
              <button type='submit' className='cursor-pointer lg:[w-10%] bg-gradient-to-r from-yellow-400 via-orange-500 to-pink-500 text-white px-16 py-3 text-sm shadow-md transition-transform duration-200 hover:scale-105 hover:from-yellow-700 hover:to-pink-700 focus:outline-none focus:ring-2 focus:ring-black active:scale-95'>PLACE ORDER</button>
          </div>
        </div>
      </div>
    </form>
